...
def _schedule_replace_and_restart(executable: Path, downloaded: Path) -> None:
    ...
    script_lines = [
        ...
        "    $envSnapshot['USERPROFILE'] = [System.Environment]::GetFolderPath('UserProfile')",
        "    Write-Log 'USERPROFILE was missing; restored from system profile.'",
        "}",
        "if (-not $envSnapshot.ContainsKey('APPDATA')) {",
        "    $envSnapshot['APPDATA'] = [System.Environment]::GetFolderPath('ApplicationData')",
        "    Write-Log 'APPDATA was missing; restored from system profile.'",
        "}",
        "if (-not $envSnapshot.ContainsKey('LOCALAPPDATA')) {",
        "    $envSnapshot['LOCALAPPDATA'] = [System.Environment]::GetFolderPath('LocalApplicationData')",
        "    Write-Log 'LOCALAPPDATA was missing; restored from system profile.'",
        "}",
