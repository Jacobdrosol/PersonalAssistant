...
def _schedule_replace_and_restart(executable: Path, downloaded: Path) -> None:
    ...
    script_lines = [
        ...
        "        if ($startInfo.EnvironmentVariables.ContainsKey($var)) {",
        "            Write-Log ('Removing environment variable {0} for child.' -f $var)",
        "            $startInfo.EnvironmentVariables.Remove($var)",
        "        }",
        "    }",
        ...
        "        }",
        "    }",
        "    foreach ($var in $pyiVars) { $null = $startInfo.EnvironmentVariables.Remove($var) }",
