...
def _schedule_replace_and_restart(executable: Path, downloaded: Path) -> None:
    ...
    script_lines = [
        ...
        "        try {",
        "            # Create temp batch file to preserve environment variables",
        "            $batchPath = [System.IO.Path]::ChangeExtension($ScriptPath, '.bat')",
        "            $batchContent = '@echo off\r\nsetlocal enabledelayedexpansion\r\n'",
        "            Get-ChildItem Env: | Where-Object { $_.Name -like '_MEI*' } | ForEach-Object {",
        "                $batchContent += \"SET \"$($_.Name)=$($_.Value -replace '\"','\"\"')\"\r\n\"",
        "                Write-Log ('Preserving environment variable: {0}={1}' -f $_.Name, $_.Value)",
        "            }",
        "            if ($argumentList.Count -gt 0) {",
        "                $argsString = ($argumentList | ForEach-Object { '\\\"' + $_ + '\\\"' }) -join ' '",
        "                $batchContent += \"\\\"$TargetPath\\\" $argsString %*\" + \"\r\n\"",
        "            } else {",
        "                $batchContent += \"\\\"$TargetPath\\\" %*\" + \"\r\n\"",
        "            }",
        "            Set-Content -Path $batchPath -Value $batchContent -Encoding ASCII",
        "            $proc = Start-Process -FilePath $batchPath -WorkingDirectory $workingDirectory -WindowStyle Normal -PassThru -ErrorAction Stop",
        "        } catch {",
