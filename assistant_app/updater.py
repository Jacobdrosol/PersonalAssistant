...
def _schedule_replace_and_restart(executable: Path, downloaded: Path) -> None:
    ...
    script_lines = [
        ...
        "$pyiVars = @('_MEIPASS', '_MEIPASS2', '_PYI_TEMP_DIR', '_PYI_SHIMS_CACHE_DIR', '_PYI_CACHE_DIR')",
        "foreach ($var in $pyiVars) {",
        "    $envPath = \"Env:$var\"",
        "    if (Test-Path $envPath) {",
        "        Write-Log ('Removing environment variable {0}.' -f $var)",
        "        Remove-Item $envPath -ErrorAction SilentlyContinue",
        "    }",
        "}",
        "Set-Item Env:PYI_SAFE_MODE '1' -ErrorAction SilentlyContinue",
        "Set-Item Env:PYI_ENV_CLEANED '1' -ErrorAction SilentlyContinue",
        "$profilePath = [System.Environment]::GetFolderPath('UserProfile')",
        "if (-not $profilePath) { $profilePath = $env:USERPROFILE }",
        "if ($profilePath) {",
        "    if (-not $env:USERPROFILE) { Set-Item Env:USERPROFILE $profilePath -ErrorAction SilentlyContinue; Write-Log ('USERPROFILE restored to {0}.' -f $profilePath) }",
        "    if (-not $env:HOME) { Set-Item Env:HOME $profilePath -ErrorAction SilentlyContinue; Write-Log ('HOME restored to {0}.' -f $profilePath) }",
        "    if (-not $env:HOMEDRIVE -and ($profilePath -match '^[A-Za-z]:')) { Set-Item Env:HOMEDRIVE $profilePath.Substring(0,2) -ErrorAction SilentlyContinue }",
        "    if (-not $env:HOMEPATH -and $profilePath.Length -gt 2) { Set-Item Env:HOMEPATH $profilePath.Substring(2) -ErrorAction SilentlyContinue }",
        "}",
        "$appDataPath = [System.Environment]::GetFolderPath('ApplicationData')",
        "if ($appDataPath -and -not $env:APPDATA) { Set-Item Env:APPDATA $appDataPath -ErrorAction SilentlyContinue; Write-Log ('APPDATA restored to {0}.' -f $appDataPath) }",
        "$localAppDataPath = [System.Environment]::GetFolderPath('LocalApplicationData')",
        "if ($localAppDataPath -and -not $env:LOCALAPPDATA) { Set-Item Env:LOCALAPPDATA $localAppDataPath -ErrorAction SilentlyContinue; Write-Log ('LOCALAPPDATA restored to {0}.' -f $localAppDataPath) }",
        "$env:PA_UPDATE_LAUNCH_TIME = (Get-Date).ToString('o')",
        "$childEnvCount = [System.Environment]::GetEnvironmentVariables().Count",
        "Write-Log ('Child environment entries: {0}' -f $childEnvCount)",
