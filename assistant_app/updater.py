...
def _schedule_replace_and_restart(executable: Path, downloaded: Path) -> None:
    ...
    script_lines = [
        ...
        "Dim shell, fso, wmi, processes, attempt, sourceSize, targetSize",
        ...
        "Loop",
        "sourceSize = -1",
        "If fso.FileExists(SOURCE) Then sourceSize = fso.GetFile(SOURCE).Size",
        "For attempt = 1 To MAX_RETRIES",
        "    Err.Clear",
        "    fso.CopyFile SOURCE, TARGET, True",
        "    If Err.Number = 0 Then",
        "        If fso.FileExists(TARGET) Then",
        "            targetSize = fso.GetFile(TARGET).Size",
        "            If sourceSize = -1 Or targetSize = sourceSize Then Exit For",
        "        End If",
        "    End If",
        "    Err.Clear",
        "    WScript.Sleep RETRY_DELAY",
        "Next",
        "If Err.Number = 0 And fso.FileExists(TARGET) Then",
