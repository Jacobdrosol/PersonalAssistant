...
def _schedule_replace_and_restart(executable: Path, downloaded: Path) -> None:
    ...
    script_lines = [
        ...
        "Write-Log ('Working directory set to: {0}' -f $workingDirectory)",
        "$pyiVars = @('_MEIPASS', '_MEIPASS2', 'PYI_TMPDIR', 'PYI_TEMP_DIR', '_PYI_SHIMS_CACHE_DIR', '_PYI_CACHE_DIR')",
        "foreach ($var in $pyiVars) {",
        "    $envPath = \"Env:$var\"",
        "    if (Test-Path $envPath) {",
        "        Write-Log ('Clearing environment variable {0} before relaunch.' -f $var)",
        "        try { Remove-Item $envPath -ErrorAction SilentlyContinue } catch {}",
        "    }",
        "}",
        "[Environment]::SetEnvironmentVariable('PYI_TMPDIR', $null, 'Process')",
        "[Environment]::SetEnvironmentVariable('PYI_SAFE_MODE', '1', 'Process')",
        "$stagingRoot = Join-Path ([System.IO.Path]::GetTempPath()) ('pa-update-run-' + [System.Guid]::NewGuid().ToString('N'))",
        "Write-Log ('Using staging directory {0} for PyInstaller unpack.' -f $stagingRoot)",
        "try { New-Item -ItemType Directory -Path $stagingRoot -Force | Out-Null } catch { Write-Log ('Failed to create staging directory: {0}' -f $_.Exception.Message); $stagingRoot = $null }",
        "if ($stagingRoot) { [Environment]::SetEnvironmentVariable('PYI_TMPDIR', $stagingRoot, 'Process') }",
        "[Environment]::SetEnvironmentVariable('PYI_SAFE_MODE','1','Process')",
        "try {",
        "    $maxLaunchAttempts = 3",
        ...
        "    }",
        "    if ($launchSuccess -and $stagingRoot -and (Test-Path -LiteralPath $stagingRoot)) {",
        "        Write-Log ('Cleaning staging directory {0}.' -f $stagingRoot)",
