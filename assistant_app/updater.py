...
def _schedule_replace_and_restart(executable: Path, downloaded: Path) -> None:
    ...
    script_lines = [
        ...
        "try {",
        "    # Preserve PyInstaller environment variables",
        "    $pyVars = [System.Environment]::GetEnvironmentVariables().Keys | Where-Object { $_ -like '_MEI*' }",
        "    foreach ($var in $pyVars) {",
        "        $value = [System.Environment]::GetEnvironmentVariable($var)",
        "        Write-Log ('Preserved environment variable: {0}={1}' -f $var, $value)",
        "    }",
        "    $maxLaunchAttempts = 3",
        ...
        "        try {",
        "            $envVars = @{}",
        "            Get-ChildItem Env: | Where-Object { $_.Name -like '_MEI*' } | ForEach-Object { $envVars[$_.Name] = $_.Value }",
        "            Write-Log ('Launching with environment variables: {0}' -f ($envVars.Keys -join ', '))",
        "            if ($argumentList.Count -gt 0) {",
        "                $proc = Start-Process -FilePath $TargetPath -ArgumentList $argumentList -WorkingDirectory $workingDirectory -WindowStyle Normal -PassThru -ErrorAction Stop -EnvironmentVariables $envVars",
        "            } else {",
        "                $proc = Start-Process -FilePath $TargetPath -WorkingDirectory $workingDirectory -WindowStyle Normal -PassThru -ErrorAction Stop -EnvironmentVariables $envVars",
        "            }",
        "        } catch {",
