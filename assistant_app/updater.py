...
def _schedule_replace_and_restart(executable: Path, downloaded: Path) -> None:
    ...
    script_lines = [
        ...
        "    }",
        "    Write-Log ('Launch command: {0}' -f $launchCmd)",
        "    $proc = Start-Process -FilePath 'cmd.exe' -ArgumentList '/c', $launchCmd -WorkingDirectory $workingDirectory -WindowStyle Normal -PassThru -ErrorAction Stop",
        "    if ($null -ne $proc) {",
