...
def _schedule_replace_and_restart(executable: Path, downloaded: Path) -> None:
    ...
    script_lines = [
        ...
        "$envSnapshot = @{}",
        "[System.Environment]::GetEnvironmentVariables('Process').GetEnumerator() | ForEach-Object { $envSnapshot[$_.Key] = $_.Value }",
        "foreach ($var in $pyiVars) {",
        "    if ($envSnapshot.ContainsKey($var)) {",
        "        Write-Log ('Removing environment variable {0} for child process.' -f $var)",
        "        $envSnapshot.Remove($var) | Out-Null",
        "    }",
        "    $envPath = \"Env:$var\"",
        ...
        "    }",
        "}",
        "$envSnapshot['PYI_SAFE_MODE'] = '1'",
        "$envSnapshot['PYI_ENV_CLEANED'] = '1'",
        "$profilePath = [System.Environment]::GetFolderPath('UserProfile')",
        "if (-not $profilePath) { $profilePath = $env:USERPROFILE }",
        "if ($profilePath) {",
        "    if (-not $envSnapshot.ContainsKey('USERPROFILE')) {",
        "        $envSnapshot['USERPROFILE'] = $profilePath",
        "        Write-Log ('USERPROFILE restored to {0}.' -f $profilePath)",
        "    }",
        "    if (-not $envSnapshot.ContainsKey('HOME')) {",
        "        $envSnapshot['HOME'] = $profilePath",
        "        Write-Log ('HOME restored to {0}.' -f $profilePath)",
        "    }",
        "    if (-not $envSnapshot.ContainsKey('HOMEDRIVE') -and ($profilePath -match '^[A-Za-z]:')) {",
        "        $envSnapshot['HOMEDRIVE'] = $profilePath.Substring(0,2)",
        "    }",
        "    if (-not $envSnapshot.ContainsKey('HOMEPATH')) {",
        "        $envSnapshot['HOMEPATH'] = $profilePath.Substring(2)",
        "    }",
        "}",
        "$appDataPath = [System.Environment]::GetFolderPath('ApplicationData')",
        "if ($appDataPath -and -not $envSnapshot.ContainsKey('APPDATA')) {",
        "    $envSnapshot['APPDATA'] = $appDataPath",
        "    Write-Log ('APPDATA restored to {0}.' -f $appDataPath)",
        "}",
        "$localAppDataPath = [System.Environment]::GetFolderPath('LocalApplicationData')",
        "if ($localAppDataPath -and -not $envSnapshot.ContainsKey('LOCALAPPDATA')) {",
        "    $envSnapshot['LOCALAPPDATA'] = $localAppDataPath",
        "    Write-Log ('LOCALAPPDATA restored to {0}.' -f $localAppDataPath)",
        "}",
        "$envSnapshot['PA_UPDATE_LAUNCH_TIME'] = (Get-Date).ToString('o')",
        "Write-Log ('Child environment entries: {0}' -f $envSnapshot.Count)",
