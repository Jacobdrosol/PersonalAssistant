...
class ContactTab(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(header, text="Contact & Feedback", style="SidebarHeading.TLabel").pack(anchor="w")
        ttk.Label(
            header,
        ...
        ttk.Label(footer, textvariable=self.status_var, foreground="#9FA8DA").pack(side=tk.LEFT)
    ...
    def _deliver_via_smtp(self, settings: SmtpSettings, message: EmailMessage) -> None:
        ...
        self._log("SMTP transaction completed without error.")

    # ------------------------------------------------------------------ Utility helpers
    def _load_smtp_settings(self) -> SmtpSettings:
        ...
        if not host or not username or not password:
            raise RuntimeError(
                "Support email is not configured and Outlook automation is unavailable."
            )
        ...
        from_address = os.getenv("PA_SUPPORT_FROM_ADDRESS", username).strip()
        return SmtpSettings(
            host=host,
            port=port,
            username=username,
